...
export default function FloaTasksPage() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < sorted.length; i++) {
      for (let j = i + 1; j < sorted.length; j++) {
        ...
        const aEnd = a.start + (a.dur || 0), bEnd = b.start + (b.dur || 0);
        if (a.start < bEnd && aEnd > b.start && a.dur && b.dur) warns.push({ a: a.id, b: b.id });
      }
    }
  ...
  // Layout constants
  const TIME_X  = 12;   // left edge of time labels
  const LINE_X  = 78;   // vertical timeline line  (moved left, more space)
  const NODE_W  = 30;
  const NODE_L  = LINE_X - NODE_W / 2;
  const CARD_L  = LINE_X + NODE_W / 2 + 12;
    ...
    <div style={{ display: "flex", height: "100vh", background: "#f8f7ff", fontFamily: "'DM Sans',-apple-system,sans-serif", color: "#111827", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", position: "relative", background: "#fff" }}>
        ...
        <div style={{ padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid #f3f4f6", background: "#fff", position: "sticky", top: 0, zIndex: 50, gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <button onClick={() => goToDate(-1)}
              style={{ width: 30, height: 30, borderRadius: 8, border: "1px solid #e5e7eb", background: "#f9fafb", color: "#6b7280", cursor: "pointer", fontSize: 15, display: "flex", alignItems: "center", justifyContent: "center" }}>‹</button>
            <div style={{ position: "relative" }} ref={calRef}>
              <button onClick={() => setShowCal(p => !p)}
                style={{ background: "none", border: "none", cursor: "pointer", textAlign: "left", padding: "2px 4px", borderRadius: 8 }}>
            ...
            {!isToday && (
              <button onClick={() => setSelectedDate(TODAY)}
                style={{ padding: "5px 12px", borderRadius: 8, border: "1px solid #e5e7eb", background: "#f9fafb", color: "#6b7280", cursor: "pointer", fontSize: 11, fontWeight: 600, fontFamily: "inherit" }}>Today</button>
            )}
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <button onClick={() => openAdd(null)}
              style={{ padding: "9px 18px", borderRadius: 10, border: "none", background: "#6366f1", color: "#fff", cursor: "pointer", fontSize: 13, fontWeight: 700, fontFamily: "inherit" }}>
              + New task
            </button>
          </div>
        </div>
        ...
        <div ref={tlRef} style={{ position: "relative", padding: "0 20px 100px 0", height: tlHeight }}>
          ...
          {hours.map(h => {
            ...
            return (
              <div key={h} style={{ position: "absolute", top: y, left: 0, right: 0, pointerEvents: "none" }}>
                {label && (
                  <span style={{
                    position: "absolute", left: TIME_X, width: TIME_W - TIME_X, textAlign: "left",
                    fontSize: 10, color: isMajor ? "#9ca3af" : "#d1d5db",
                    lineHeight: 1, transform: "translateY(-50%)",
                    fontWeight: isMajor ? 600 : 400, whiteSpace: "nowrap",
                  }}>{label}</span>
                )}
          ...
          {/* TASKS */}
          {(() => {
            // Compute overlap columns: tasks that time-overlap get pushed right
            const colMap = {}; // taskId → column index (0 = normal, 1 = shifted right)
            ...
            const COL_SHIFT = 52; // px shift per column
            ...
            const isOvlp   = overlapWarnings.some(w => w.a === task.id || w.b === task.id);
            const totalSubs= (task.subtasks || []).length;
            const doneSubs = (task.subtasks || []).filter(s => s.done).length;
              ...
              <div key={task.id} className="task-row"
                ...
                <div
                  ...
                  style={{
                    ...
                    borderRadius: hasDur ? 15 : "50%",
                    background: isDone ? "#dcfce7" : isPast ? col.light : col.light,
                    border: `2px solid ${isNow || isSoon ? col.hex : isDone ? "#22c55e" : isPast ? col.hex + "44" : col.hex + "66"}`,
                    overflow: "hidden",
                    cursor: isTLDrag ? "grabbing" : "grab",
                    boxShadow: isNow ? `0 0 0 4px ${col.hex}22` : isSoon ? `0 0 0 3px ${col.hex}15` : "none",
                    userSelect: "none",
                    outline: isOvlp ? `2px solid #fbbf24` : "none",
                    outlineOffset: 2,
                  }}>
                  {fillFrac > 0 && (
                    <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: `${fillFrac * 100}%`, background: isDone ? "#86efac" : col.hex + "66", transition: "height 1.5s linear" }} />
                  )}
                  <div style={{ position: "absolute", top: hasDur ? 8 : 0, left: 0, right: 0, bottom: hasDur ? 16 : 0, display: "flex", alignItems: hasDur ? "flex-start" : "center", justifyContent: "center", fontSize: 14, zIndex: 2, paddingTop: hasDur ? 2 : 0, filter: isDone ? "grayscale(1) opacity(0.5)" : isPast ? "opacity(0.7)" : "none" }}>
                    {task.icon}
                  </div>
                  {hasDur && (
                    <div style={{ position: "absolute", bottom: 5, left: 0, right: 0, display: "flex", justifyContent: "center", zIndex: 2 }}>
                      <div style={{ width: 12, height: 12, borderRadius: 3, background: isDone ? "#22c55e" : col.hex + "33", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 7, fontWeight: 700, color: isDone ? "#fff" : col.hex }}>✓</div>
                    </div>
                  )}
                </div>
                ...
                <div style={{ position: "absolute", left: CARD_L, right: 36, top: 0, zIndex: isExp ? 20 : "auto" }}>
                  {/* Opaque backdrop when expanded so nothing beneath bleeds through */}
                  {isExp && <div style={{ position: "absolute", inset: 0, background: "#fff", borderRadius: 12, zIndex: -1, bottom: "auto", top: 0, left: -4, right: -4 }} />}
                  <div
                    onMouseDown={e => beginDrag(e, task, true)}
                    style={{
                    ...
                    <div style={{ fontSize: 10, fontWeight: isNow || isSoon ? 700 : 500, color: isNow || isSoon ? col.hex : isDone ? "#d1d5db" : isPast ? col.hex + "99" : "#9ca3af", marginBottom: 2, lineHeight: 1.2 }}>
                      {isNow && hasDur
                    ...
                    <div style={{ fontSize: 14, fontWeight: 600, color: isDone ? "#16a34a" : isPast ? col.text : "#111827", lineHeight: 1.3, position: "relative", display: "inline-block" }}>
                      {task.name}
                      {isDone && (
                        <div style={{ position: "absolute", top: "50%", left: 0, height: 2, width: "100%", background: "#22c55e", borderRadius: 1, transform: "translateY(-50%)", animation: "strikeIn .35s ease-out both" }} />
                      )}
                    </div>
                    ...
                    {!isExp && <div style={{ width: 18, height: 2.5, borderRadius: 2, background: col.hex, marginTop: 5, opacity: isDone ? 0.2 : 0.7 }} />}
                  </div>
                ...
                <div style={{ position: "absolute", right: 0, top: hasDur ? 10 : 5 }}>
                  <button onClick={e => { e.stopPropagation(); toggleDone(task.id); }}
                    ...
                    style={{ width: 22, height: 22, borderRadius: "50%", border: `2px solid ${isDone ? "#22c55e" : "#d1d5db"}`, background: isDone ? "#22c55e" : "#fff", cursor: "pointer", transition: "border-color .2s, background .2s", display: "flex", alignItems: "center", justifyContent: "center" }}
                    onMouseEnter={e => e.currentTarget.style.transform = "scale(1.15)"}
                    onMouseLeave={e => e.currentTarget.style.transform = "scale(1)"}>
                    {isDone && <span className="done-check" style={{ fontSize: 10, color: "#fff", fontWeight: 800, lineHeight: 1 }}>✓</span>}
                  </button>
                </div>
        ...
        <div style={{ padding: "16px 14px 12px", borderBottom: "1px solid #e5e7eb", background: "#fff" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
            ...
            <div style={{ display: "flex", gap: 4 }}>
              <button onClick={() => setActivePanelIdx(i => (i - 1 + panels.length) % panels.length)}
                style={{ width: 26, height: 26, borderRadius: 7, border: "1px solid #e5e7eb", background: "#f9fafb", color: "#6b7280", cursor: "pointer", fontSize: 13 }}>‹</button>
              <button onClick={() => setActivePanelIdx(i => (i + 1) % panels.length)}
                style={{ width: 26, height: 26, borderRadius: 7, border: "1px solid #e5e7eb", background: "#f9fafb", color: "#6b7280", cursor: "pointer", fontSize: 13 }}>›</button>
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: 5 }}>
            {panels.map((p, i) => (
              <button key={p.id} onClick={() => setActivePanelIdx(i)}
                style={{ flex: 1, height: 3, borderRadius: 2, border: "none", cursor: "pointer", background: i === activePanelIdx ? "#6366f1" : "#e5e7eb", transition: "background .2s" }} />
            ))}
          </div>
      ...
      {drag && drag.moved && (
        <div style={{ position: "fixed", left: drag.x + 16, top: drag.y - 16, pointerEvents: "none", zIndex: 1000,
          padding: "7px 13px", borderRadius: 10, background: "#fff", border: "1px solid #e5e7eb", color: "#111827", fontSize: 13,
          display: "flex", alignItems: "center", gap: 9, boxShadow: "0 8px 36px rgba(0,0,0,0.15)", whiteSpace: "nowrap" }}>
